...
!
  ...
  !---- 時間積分パラメター ----
    real(8):: dt                                   ! 時間間隔 [s]
    type(DC_DIFFTIME):: deltime, dispint           ! 時間間隔, 出力時間間隔
  ...
  !---- 時間積分パラメターの設定 ----
    dt = 100.                                      ! 時間間隔 [s]
    call DCDiffTimeCreate(deltime, dt, 'sec')      ! 時間間隔
    dispint = deltime * 500                        ! 出力時間間隔
    endtime = deltime * 10000                      ! 計算終了時間
    call DCDiffTimeCreate(ct,      0., 'sec')      ! 現在時刻
  ...
  !------------------- 時間積分 ----------------------
    do while ( ct <= endtime )
       ...
       if ( mod(ct, dispint) == 0 ) then                   ! 出力
          call output_gtool5
       end if
    enddo

    call output_gtool5_close
    stop

  contains
    subroutine output_gtool5_init
      call HistoryCreate( &                               ! ヒストリー作成
           file='sp_topo_gthist.nc',                               &
           ...
           origin=0.0, interval=real(EvalSec(dispint)) )
